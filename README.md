...
This project is an IT Services Chatbot designed to assist users by identifying their IT service needs and collecting their contact information. It integrates with Google's Gemini API (and potentially OpenAI for fallback or specific features) for natural language understanding and conversation, as well as for extracting user details from the chat. The application then sends an email notification with the inquiry details to the company and a confirmation email to the user.
...
*   **Google Gemini Integration:**
    *   Uses `gemini-2.0-flash-lite` for general chatbot conversation and for structured extraction of user details (name, email, phone, company, service, requirements) using its function calling capabilities.
...
*   An active [Google AI API Key](https://aistudio.google.com/app/apikey) (for Gemini).
*   (Optional) An active [OpenAI API Key](https://platform.openai.com/api-keys) if you plan to use OpenAI models alongside or as a fallback.
...
2.  **Install Dependencies:**
    ...
    npm install
    ```
    This will install all the necessary packages defined in `package.json` (e.g., Express, @google/generative-ai, Nodemailer, Cors, Dotenv).
...
4.  **Configure Environment Variables:**
    ...
    OPENAI_API_KEY=your_openai_api_key_here # Keep if you still use OpenAI for any part
    ...
    *   `OPENAI_API_KEY`: (Optional) Your secret API key from OpenAI.
...
*   `POST /api/send-inquiry`
    *   **Description:** Triggered when the chatbot conversation indicates completion of information gathering. This endpoint uses the Google Gemini API with function calling to extract structured details from the conversation history and then sends the internal and user confirmation emails.
...
*   **AI API Errors (e.g., "Incorrect API key provided", "model_not_found", "PermissionDeniedError"):**
    *   Verify that `GEMINI_API_KEY` (and `OPENAI_API_KEY` if used) in your `.env` file is correct and active.
    *   Ensure your Google AI account (and OpenAI account if used) has sufficient credits or is on an appropriate plan for the models being used.
    *   The application is configured to use `gemini-2.0-flash-lite`. If you encounter errors, ensure your project has access to this model.
